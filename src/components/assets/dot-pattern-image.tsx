// biome-ignore-all lint/style/noNonNullAssertion: someone else's code
"use client"
...
export type DotPatternImageProps = {
  src: string
...
export const DotPatternImage: React.FC<DotPatternImageProps> = ({
  className = "",
  src,
  height,
  width,
  ...
  const darkMode = theme.resolvedTheme === "dark"
  ...
  useEffect(() => {
    ...
    function render(time: number) {
      ...
      mouseActivationRef.current +=
        (targetActivation - mouseActivationRef.current) * activationSpeed
  ...
  return (
    <canvas
      className={cn("pointer-coarse:pointer-events-none bg-transparent", className)}
      height={height}
      ref={canvasRef}
      width={width}
    />
...
export default DotPatternImage
...
void main() {
  ...
  float dotSizePixels = 2.0;
  float spacingPixels = 5.0;
  ...
  // Adjust dot size based on brightness (in pixels) - darker areas get larger dots
  float baseDotSize = dotSizePixels * smoothstep(0.2, 0.8, (1.0 - brightness));
  ...
  if (adjustedOriginalDotCenter.x >= 0.0 && adjustedOriginalDotCenter.x < gridCount.x * spacingPixels &&
    ...
    float baseRadius = 40.0; // Base influence radius
  ...
  vec4 dotColor = vec4(0.17455, 0.40724, 0.42356, 1);
